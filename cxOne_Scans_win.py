...
# def get_refresh_token():
#     refresh_token = os.getenv('CX_API_KEY')
#     if refresh_token is None:
#         raise ValueError('CX_API_KEY environment variable not set.')
#     return refresh_token
...
def main():
    ...
    try:
        ...
        # Create separate workbooks for jsondump and results
        workbook_jsondump = xlsxwriter.Workbook(excel_file_path)
        workbook_results = xlsxwriter.Workbook(excel_file_path)

        # Write normalized_scans to jsondump sheet
        worksheet_jsondump = workbook_jsondump.add_worksheet('jsondump')
        df = pd.DataFrame(normalized_scans)
        df.to_excel(workbook_jsondump, sheet_name='jsondump', index=False)

        # Write normalized_scans to results sheet
